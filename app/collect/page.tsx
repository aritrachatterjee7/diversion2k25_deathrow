      ...
      try {
        ...
        if (parsedResult.wasteTypeMatch && parsedResult.confidence > 0.7) {
          ...
          if (parsedResult.quantityMatch || parsedResult.confidence > 0.6) {
            ...
            toast.error('Verification failed. The collected waste quantity does not match the reported waste.', {
              duration: 5000,
              position: 'top-center',
            })
          }
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="flex justify-end">
        {task.status === 'pending' && (
          <Button onClick={() => onStatusChange(task.id, 'in_progress')} variant="outline" size="sm">
            Start Collection
          </Button>
        )}
        {task.status === 'in_progress' && task.collectorId === user?.id && (
          <Button onClick={() => onSelectTask(task)} variant="outline" size="sm">
            Complete & Verify
          </Button>
        )}
        {task.status === 'in_progress' && task.collectorId !== user?.id && (
